...
model_path = r"emotion_detection_model.h5"
...
#     img = np.expand_dims(np.array(img), axis=0).reshape(1, 48, 48, 1)
#     prediction = model.predict(img)
#     # predicted_emotion = emotion_mapping.get(np.argmax(prediction), "Unknown")
#     return prediction
...
# Emotion output function
def out_put(predicted_emotion):
    if predicted_emotion == "Anger":
        return random.choice(teenager_angry)
    elif predicted_emotion == "Sad":
        return random.choice(teenager_sad)
    elif predicted_emotion == "Happy":
        return random.choice(teenager_happy)
    elif predicted_emotion == "Contempt":
        return random.choice(teenager_neutral)
    elif predicted_emotion == "Fear":
        return random.choice(teenager_fearful)
    elif predicted_emotion == "Surprised":
        return random.choice(teenager_surprised)
    elif predicted_emotion == "Disgust":
        return random.choice(teenager_disgusted)
...
def upload():
    try:
        ...
        uploads_dir = os.path.join(views.root_path,'uploads')
        os.makedirs(uploads_dir, exist_ok=True)

        # Define file path and save image
        image_path = os.path.join(uploads_dir,'captured_image.jpeg')
        with open(image_path, 'wb') as f:
            f.write(image_data)

        # Predict the emotion and generate response
